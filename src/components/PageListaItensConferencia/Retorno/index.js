...
export default function ListagemItensRetorno({ itensConferencia, navigation, route }) {
  const { idDemanda } = route.params;
  const [pesquisar, setPesquisar] = useState("");
  const filtro = Helpers.filterAndGetProducts(itensConferencia).filter((filtrar) => filtrar.motivo === "Devolução");
